...
export class OrderDetailService extends BaseDetailCrud<IOrder> {
    ...
    private regexCheck(order: IOrder) {
        ...
        // --- 1. ПРИОРИТЕТ: SAVED SETTINGS (Ръчно избрани от оператора) ---
        if (saved && saved.courierId) {
            shipmentType = saved.courierShipmentType;
            courierType = saved.courierType;
            const office = saved.office as any;
            if (office) targetId = office.code || office.id || '';
        }
        // --- 2. ВТОРИ ПРИОРИТЕТ: REGEX ВЪРХУ АДРЕСА ---
        else {
            const addr = (shipping?.address_1 || billing?.address_1 || '').trim();
            ...
            // --- 3. FALLBACK: SHIPPING LINES (Ако адресът е чист текст) ---
            // Това е частта, която липсваше или беше непълна
            if (!courierType && shippingLine) {
                const title = shippingLine.method_title.toLowerCase();

                // Определяне на куриер
                if (title.includes('econt')) courierType = 'ECONT';
                else if (title.includes('speedy')) courierType = 'SPEEDY';
                else if (title.includes('boxnow') || title.includes('box now')) courierType = 'BOX_NOW';

                // Определяне на тип (Shipment Type)
                if (title.includes('aдрес') || title.includes('адрес')) {
                    shipmentType = 'ADDRESS';
                } else if (title.includes('автомат') || title.includes('locker') || courierType === 'BOX_NOW') {
                    shipmentType = 'LOCKER';
                } else {
                    shipmentType = 'OFFICE'; // По подразбиране
                }
            }
        }

        // Финална корекция за Java Enum
        if (courierType === 'BOXNOW') courierType = 'BOX_NOW';

        // Ако все още е null, сложи OFFICE по подразбиране, за да не гърми калкулатора
        // if (courierType && !shipmentType) shipmentType = 'OFFICE';
