...
import { WpBrandListService } from './list.service';
...
@Component({
    ...
    template: `
        ...
        >
            ...
            <ng-template pTemplate="body" let-item>
                <tr [ngClass]="{ 'cursor-pointer hover:bg-blue-50': this.config?.data?.mode === 'lookup' }">
                    ...
                    <td>{{ item.slug }}</td>
                    <td>
                         <span [pTooltip]="item.description"
                            tooltipPosition="top"
        ...
        <wp_brand-detail *ngIf="config?.data?.mode !== 'lookup'"></wp_brand-detail>
    `
})
export class WpBrandListComponent {
    public listService = inject(WpBrandListService);
    public detailService = inject(WpBrandDetailService);
    private tr = inject(TranslateService);
    protected config = inject(DynamicDialogConfig, { optional: true });

    selectedItem!: IWpBrand[] | null;
